export const title = "Creative Works";

export const onMount = (params) => {};

export const html = /* html */ `
    <div class="creative-works-hub">
            <a class="creative-works-hub-link" href="/#/creative-works/art">Art</a>
            <a class="creative-works-hub-link" href="/#/creative-works/poetry">Poetry</a>
            <a class="creative-works-hub-link" href="/#/creative-works/other-writing">Other Writing</a>
            <a class="creative-works-hub-link" href="/#/creative-works/music">Music</a>
    </div>
    <hr>
    <p>
        All posted creative works, including but not limited to art, poetry, and prose, 
        are subject to the <a href="https://creativecommons.org/licenses/by-nc-sa/4.0/" target="_blank">CC BY-NC-SA-4.0</a> license. 
    </p>
    <p>
        All of my music is subject to a more permissive <a href="https://creativecommons.org/licenses/by-sa/4.0/">CC BY-SA-4.0</a> license.
    </p>
    <p>
        Original Characters in fanarts and works posted on the 'Art' page belong to their original creators.
    </p>
    <p>
        Feel free to make fanarts of my characters if you so desire, I would really appreciate that, because I love them to bits!
    </p>
`;
